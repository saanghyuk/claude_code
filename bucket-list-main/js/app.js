...
class BucketListApp {
    ...
    createBucketItemHTML(item) {
        ...
        const completedItemClass = item.completed ? 'completed-item' : '';
        ...
        return `
            <div class="bucket-item ${completedItemClass} p-5 flex items-center gap-4">
                <!-- 체크박스 -->
                <button
                    class="flex-shrink-0 w-7 h-7 rounded-lg border-2 flex items-center justify-center ${checkboxClass}"
                ...
                <div class="flex gap-2 flex-shrink-0">
                    <button
                        class="edit-btn px-3 py-1.5 bg-blue-50 text-blue-600 text-sm font-medium"
                        onclick="app.openEditModal('${item.id}', '${this.escapeHtml(item.title).replace(/'/g, "\\'")}')"
                    >
                        수정
                    </button>
                    <button
                        class="delete-btn px-3 py-1.5 bg-red-50 text-red-500 text-sm font-medium"
